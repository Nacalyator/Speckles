...
base_dir = './'
data_dir = os.path.join(base_dir, 'train_v4')
...
for i in range(len(frames)-1):
	for j in range(1, 7):
		...
		if j == 1:
			...
			frame_2 = cv2.rotate(frames[i + 1], cv2.ROTATE_90_CLOCKWISE)
		elif j == 3:
			frame_1 = cv2.rotate(frames[i], cv2.ROTATE_180)
			frame_2 = cv2.rotate(frames[i + 1], cv2.ROTATE_180)
		elif j == 4:
			frame_1 = cv2.rotate(frames[i], cv2.ROTATE_90_COUNTERCLOCKWISE)
			frame_2 = cv2.rotate(frames[i + 1], cv2.ROTATE_90_COUNTERCLOCKWISE)
		elif j == 5:
			frame_1 = cv2.flip(frames[i], 0)
			frame_2 = cv2.flip(frames[i + 1], 0)
		elif j == 6:
			frame_1 = cv2.flip(frames[i], 1)
			frame_2 = cv2.flip(frames[i + 1], 1)
		elif j == 7:
			frame_1 = cv2.flip(frames[i], -1)
			frame_2 = cv2.flip(frames[i + 1], -1)
		...
		if (count % count_val) == 0:
			...
			df_writer_val.writerow([filename, str(abs(data_deform[i] - data_deform[i+1]))])
		else:
			...
			df_writer_train.writerow([filename, str(abs(data_deform[i] - data_deform[i+1]))])
